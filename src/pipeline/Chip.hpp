...
SC_MODULE(Chip) {
  ...
  /// Decoder
  Decoder *decoder;
  sc_signal<sc_uint<4>> decoder_rs_signal, decoder_rt_signal, decoder_rd_signal;
  sc_signal<sc_uint<32>> decoder_immediate_i_signal, decoder_immediate_j_signal;
  ...
  /// MUX EX_RD
  Mux<4, 1> *mux_ex_rd;
  sc_signal<sc_uint<1>> mux_ex_rd_sel_signal;
  sc_signal<sc_uint<4>> mux_ex_rd_out_signal;

  /// Hazard Forward
  HazardForward *hazard_forward;
  sc_signal<sc_uint<2>> rs_sel_signal, rt_sel_signal;
  ...
  /// EX_MEM
  EX_MEM *ex_mem;
  sc_signal<sc_uint<32>> ex_mem_ula_result_out_signal, ex_mem_data_out_1_signal;
  ...
  void check_jump() {
    jump_pc.write(pc_out_signal.read() + decoder_immediate_j_signal.read());

    if (decoder_jump_signal.read()) {
      pc_mux_select.write(1);
    } else if (decoder_jump_n_signal.read() && ula_result_signal.read() < 0) {
      pc_mux_select.write(1);
    } else if (decoder_jump_z_signal.read() && ula_result_signal.read() == 0) {
      pc_mux_select.write(1);
    } else {
  ...
  SC_CTOR(Chip) {
    ...
    /// MUX EX_RD
    mux_ex_rd = new Mux<4, 1>("mux_ex_rd");
    mux_ex_rd->clk(clk);
    mux_ex_rd->reset(reset);
    mux_ex_rd->in[0](id_ex_rd_out);
    mux_ex_rd->in[1](id_ex_rt_out);
    ...
    ex_mem->data_in_1(id_ex_data_out_1_signal);
    ...
    sensitive << decoder_jump_signal << decoder_jump_n_signal
              << decoder_jump_z_signal << ula_result_signal;

    SC_METHOD(select_reg_dst);
    sensitive << id_ex_reg_dst_out;

    SC_METHOD(select_reg_data_write);
    sensitive << mem_wb_mem_to_reg_out_signal;
  }
