...
SC_MODULE(TestbenchMemoryUInt) {
  ...
  Memory<32, 8, sc_uint<32>> *mem;

  SC_CTOR(TestbenchMemoryUInt) : clock("clock", 10, SC_NS) {
    mem = new Memory<32, 8, sc_uint<32>>("Memory");
  ...
  void stimulus() {
    ...
    std::cout << "Valor na posição 15: " << output.read()
              << " (esperado: 4294967276)" << std::endl;
    read_enable.write(false);
...
int sc_main(int argc, char *argv[]) {
  TestbenchMemoryUInt tb("TestbenchMemoryUInt");
  sc_start();
  return 0;
}
